...
select p.first_name,p.last_name,s.province_name from patients p join province_names s on p.province_id=s.province_id;

-- 8. Show how many patients have a birth_date with 2010 as the birth year.
select count(*) as patiens_below_2010 from patients where year(birth_date)=2010;
...
select * from patients where patient_id in (1,45,534,979,1000);
...
select * from province_names;
select * from patients;
select p.first_name,p.last_name,p.city,s.province_id from patients p join province_names s on p.province_id=s.province_id where s.province_id="NS";
select first_name,last_name ,city,province_id from patients where province_id='NS';
...
select distinct year(birth_date)as birth_years from patients order by  birth_years asc; 
...
select p.first_name,p.last_name,d.diagnosis from patients p join admissions d on p.patient_id=d.patient_id where d.diagnosis='dementia';
...
select count(if (gender='M',1,"NULL"))as total_males,count(if(gender="F",1,"NULL"))as total_feamles from patients;
...
select count(if(gender="M",1,0))as total_males,count(if(gender="F",1,0))as_total_females from patients;

-- 23. Show patient_id, diagnosis from admissions. Find patients admitted multiple times for the same diagnosis.
select patient_id,diagnosis as multiple_patients from admissions group by patient_id,diagnosis having count(patient_id) >1;
...
select city ,count(*)as total_patient from patients group by city order by city asc,total_patient desc;
...
select  first_name,last_name ,"patient" as role from patients union select first_name, last_name,"doctore" as role from doctors;
...
select concat(first_name," , ",last_name)as full_name ,lower(first_name) ,upper(last_name) from patients order by first_name desc;
...
select last_name ,max(weight)-min(weight) as weight from patients where last_name ="Maroni"group by last_name;

/*31. Show all of the days of the month (1-31) and how many admission_dates occurred on that day. 
Sort by the day with most admissions to least admissions.*/
select day(admission_date)as day_of_month ,count(*)as total_admition from admissions group by admission_date order by total_admition;
...
select floor(weight/10)*10 as weight_group, count(*) from patients group by floor(weight/10)*10 order by weight_group desc ;
...
select patient_id,weight,height ,case when weight /power(height/100.0,2)>=30 then 1 else 0 end from patients ;
...
select * from admissions;
select * from province_names;
select p.patient_id,d.first_name,d.last_name,a.diagnosis ,d.specialty 
